...
| 2 | Character clustering (convolutional autoencoder) | 🚧 (`model/convolutional_autoEncoder`) Implemented but to optimize and maybe remasterize  |
...
| 5 | Cipher type classification (feature vectors + neural network) | 🚧 Tabular-only baseline implemented but not trained (`model/text_clusterization/MLP_cipher_classifier.py`); full image+tabular fusion architecture still just defined |
